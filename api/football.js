...
function operatorHandler(io, socket, msg) {
    var room = socket.rooms.filter(function(key) {
        return key.indexOf('football-') == 0;
    });
    ...
    switch (msg.action) {
        ...
        case "SEND":
            ...
            if (questionModel.containsMetadata(question)) {
                ...
                io.of('operator').to(room).emit('football', {
                    action: "EDIT",
                    data: question
                });
            } else {
    ...
    if (question && context[room]['questions'][question._id]) {
        ...
        io.of('player').to(room).emit('football', {
            uri: 'question',
            data: question
        });
        scoreHandler(room);
            ...
            if (question) {
                switch (question.status) {
                    case "live":
                        ...
                        break;
                    case "validated":
                        if ((attempt.answer == question.validation) && (attempt.timestamp < question.expires)) {
                            player.score++;
